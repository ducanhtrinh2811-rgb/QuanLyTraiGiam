...
namespace PrisonManagement.Views.Pages
{
    public partial class PhamNhanDialog : Window
    {
        ...
        private async void LoadPhongGiamAsync()
        {
            try
            {
                var phongGiams = await _apiService.GetAllPhongGiamAsync();
                cboPhongGiam.ItemsSource = phongGiams;
                if (phongGiams?.Any() == true)
                {
                    cboPhongGiam.SelectedIndex = 0;
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Lỗi load phòng giam: {ex.Message}", "Lỗi");
            }
        }
        ...
        private async void BtnSave_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (cboPhongGiam.SelectedValue == null)
            {
                MessageBox.Show("Vui lòng chọn phòng giam!", "Lỗi", MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }
